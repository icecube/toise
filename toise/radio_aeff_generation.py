...
def _load_radio_review_veff(
    ...
    bin_edges_costheta = list(
        set(
            ...
            + list(np.round(np.cos(dataframe.thetamax)))
        )
    )
...
class radio_aeff:
    ...
    def create_muon_background(
        ...
        # print(cos_theta)
        self.logger.warning(
            "Energy/direction resolution smearing not applied for atm. muons for now!"
        )
    ...
    def create(
        ...
        self.logger.debug(
            "STEP 3: Scaling trigger effective volume for number of stations and applying to density of final states"
        )
        if "nstations" in configuration["detector_setup"]:
            nstations = configuration["detector_setup"]["nstations"]
            self.logger.info("using number of stations from config")
        ...
        if configuration["apply_smearing_inelasticity"] == True:
            ...
            aeff[0:2, ...] = np.apply_along_axis(
                triggered_inelasticities.smear_energy_slice,
                3,
                aeff[0:2, ...],
                e_showering,
                "EM",
        ...
        if configuration["apply_analysis_efficiency"] == True:
            ...
            ana_efficiency = rno_analysis_efficiency(
                e_nu[:-1],
...
def combine_aeffs(aeff1, aeff2):
    ...
    if not aeff1.compatible_with(aeff2):
        ...
        def overlap(E):
            loge = [7, 7.5, 8, 8.5, 9, 9.5, 10, 10.5, 11.0]
