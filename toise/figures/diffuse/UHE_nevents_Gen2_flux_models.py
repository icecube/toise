...
np.log10(optical['shadowed_tracks'][0].bin_edges[0]), np.log10(radio['radio_events'][0].bin_edges[0])
energies = np.logspace(1,12,111)

def get_bins(energies, key, edges=edges):
    return np.isin(np.round(np.log10(energies[:-1]),2),np.round(np.log10(edges[key][:-1]),2))
...
radio_background = MuonBackground(radio["radio_events"][1], livetime).expectations.sum(0)
...
DIFFUSE_MODELS.heinze_zmax_1.plot_title = r"Heinze et al.,"\
                                           "\n"\
                                           "fit to Auger UHECRs"
DIFFUSE_MODELS.van_vliet_ta.plot_title = "Bergman & van Vliet, fit to\nTA UHECRs"
DIFFUSE_MODELS.rodrigues_bench_cosmo.plot_title = r"Rodrigues et al.,"\
                                                   "\n"\
                                                  r"all AGN (cosmogenic $\nu$)"
DIFFUSE_MODELS.rodrigues_bench_source.plot_title = r"Rodrigues et al.,"\
                                                    "\n"\
                                                   r"all AGN (source $\nu$)"
DIFFUSE_MODELS.rodrigues_hlbllacs_cosmo.plot_title = r"Rodrigues et al.,"\
                                                      "\n"\
                                                      "HL BL Lacs"
...
nbins_per_decade = 5
pltE = np.logspace(1,12,11*nbins_per_decade+1)
plt.subplots_adjust(left=0.1, bottom=0.12, right=0.999, top=0.93, wspace=0.04, hspace=0.04)
...
for i, m in enumerate(model_order):
    ...
    if i == 2:
        ...
        label_r = "radio only"
        label_1ev = "1 event per bin"
    ax[ax_list[i]].hist(10**center(np.log10(energies)), weights=plot_models[m], bins=pltE, alpha=0.4, histtype="stepfilled", linestyle="-", color=blue, linewidth=2, label=label_c)
    ax[ax_list[i]].hist(10**center(np.log10(energies)), weights=plot_models[m], bins=pltE, color="black", linestyle="-", linewidth=1, histtype="step")
    ax[ax_list[i]].hist(10**center(np.log10(energies)), weights=plot_models_radio[m], bins=pltE, color=orange, linestyle=":", linewidth=2, histtype="step", label=label_r)
    ax[ax_list[i]].hist(10**center(np.log10(energies)), weights=plot_models_optical[m], bins=pltE, color=blue, linestyle="--", linewidth=2, histtype="step", label=label_o)
    ax[ax_list[i]].loglog()
    ax[ax_list[i]].set_ylim(1e-2, 1e2)
    ...
    if i == 11:
        ...
        ax[ax_list[i]].set_xticks([1e7, 1e8, 1e9])
    ax[ax_list[i]].axhline(1, color="red", linestyle="-", label=label_1ev)
    ax[ax_list[i]].text(1.2e7,0.8e2, m.plot_title, ha='left', va='top', fontsize=14)
    ax[ax_list[i]].tick_params(axis='x', pad=8)
    #ax[ax_list[i]].legend()
fig.legend(loc=9, ncol=4, fontsize=16)
fig.supxlabel(r"Reconstructed shower energy, $E_\mathrm{sh}^\mathrm{rec}$ [GeV]",fontsize=22)
fig.supylabel(r"Number of $\nu$ events in IceCube-Gen2 ("+str(livetime)+" yr)", fontsize=22)
fig.suptitle(f'')
#fig.tight_layout()
...
for i, m in enumerate(model_order[::-1]):
    ...
    ny = float(T_5sigma(np.array([n_signal]), np.array(np.full(np.shape(n_signal), 0.3))))
    hatch = ""
    color=blue
    if ny > 10:
        hatch = "/"
