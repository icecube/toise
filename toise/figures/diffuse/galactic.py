...
def make_components(
    aeffs, galactic_emission=diffuse.FermiGalacticEmission, angular_smoothing_fwhm=None
):
    ...
    galactic_bg = RingAveraging(galactic_emission(aeff, 1), angular_smoothing_fwhm=None)
    components = dict(atmo=atmo, prompt=prompt, astro=astro, galactic=galactic, galactic_bg=galactic_bg)
    if muon_aeff:
...
def asimov_llh(
    ...
    components = create_bundle(
        exposures, galactic_emission, angular_smoothing_fwhm
    ).get_components()
    components["gamma"] = multillh.NuisanceParam(astro_gamma)
    return multillh.asimov_llh(components, astro=astro, gamma=astro_gamma, galactic_bg=0, galactic=1)
...
def fermi_pi0(
    ...
    galactic_emission: Literal["fermi", "kra_5", "kra_50"] = "fermi",
    angular_smoothing: Literal["dnn", "monopod", "none"] = "none",
    angular_resolution_scale: float = 1.,
):
    ...
    llh = asimov_llh(
        ...
        galactic_emission=galactic_emission,
        angular_smoothing_fwhm=(lambda *args: angular_resolution_scale*angular_smoothing_fwhm(*args)) if angular_smoothing_fwhm else None,
    )
    ...
    h0 = {
        k: v.sum(axis=1)
        for k, v in llh.llh_contributions(**nullhypo).items()
    }
    bestfit = nominal | {"galactic": 1, "galactic_bg": 0}
    h1 = {
        k: v.sum(axis=1)
        for k, v in llh.llh_contributions(**bestfit).items()
    }
    # sum over energies
    meta = {
        "rates": {
            ...
            for component, expectations in multillh.get_expectations(
                llh, astro=astro, gamma=astro_gamma, galactic=galactic_norm, galactic_bg=0,
            ).items()
        },
