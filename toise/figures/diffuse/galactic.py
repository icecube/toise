...
def create_bundle(exposures, galactic_emission=diffuse.FermiGalacticEmission):
    return factory.component_bundle(
        dict(exposures), partial(make_components, galactic_emission=galactic_emission)
    )
...
def fermi_pi0(exposures, astro=2.3, astro_gamma=-2.5, galactic_emission="fermi"):
    ...
    h0 = {
        k: v.sum(axis=1)
        for k, v in llh.llh_contributions(galactic=0, **nominal).items()
    }
    h1 = {
        k: v.sum(axis=1)
        for k, v in llh.llh_contributions(galactic=1, **nominal).items()
    }
    # sum over energies
    meta = {
        "rates": {
            ...
            for component, expectations in multillh.get_expectations(
                llh, astro=astro, gamma=astro_gamma, galactic=1
            ).items()
        },
    ...
    meta["signal_spectrum"] = {
        ...
        "events": multillh.get_expectations(
            llh, astro=astro, gamma=astro_gamma, galactic=1
        )["galactic"]["IceCube-TracksOnly_unshadowed_tracks"].sum(axis=0),
    }
...
def rates(datasets):
    ...
    healpy.mollview(
        bg,
        hold=True,
        title="Background (without muons): {:.0f} events".format(bg.sum()),
        unit="Events",
    )
    ...
    healpy.mollview(
        northern_sig,
        hold=True,
        title="Signal: {:.0f} events".format(northern_sig.sum()),
        unit="Events",
    )
    ...
    healpy.mollview(
        ts,
        hold=True,
        title="Discovery significance: {:.1f} $\sigma$".format(np.sqrt(ts.sum())),
        unit=r"$-2\Delta \ln L$",
    )
    ...
    ax.loglog(
        *plotting.stepped_path(
            spec["energy"], np.asarray(spec["events"])[::-1].cumsum()[::-1]
        ),
        nonposy="clip"
    )
