...
def diffuse_sensitivity_uhe():
    ...
    class units:
        ...
        second = 1
        s = second
        year = 3600 * second * 24 * 365.25

    energyBinsPerDecade = 1.0
    ...
    GRAND_200k_10 = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__), "data_gen2_limit_figure/sensitivity_grand.dat"
        )
    )
    GRAND_200k_10[:, 0] = 10 ** (GRAND_200k_10[:, 0]) * units.GeV
    GRAND_200k_10[:, 4] = 10 ** (GRAND_200k_10[:, 4]) * (
    ...
    ara_4year_E, ara_4year_limit, t1, t2 = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__), "data_gen2_limit_figure", "limit_a23.txt"
        ),
        unpack=True,
    )
    ...
    Radar = np.array(
        (
            [
                (1.525e01, 6.870e-09, 3.430e-07),
                (1.575e01, 9.797e-10, 3.113e-08),
                (1.625e01, 4.728e-09, 1.928e-07),
                (1.675e01, 6.359e-09, 3.706e-07),
                (1.725e01, 9.128e-09, 8.517e-07),
                (1.775e01, 1.619e-08, 1.835e-06),
                (1.825e01, 2.995e-08, 2.766e-06),
                (1.875e01, 5.562e-08, 8.253e-06),
                (1.925e01, 1.072e-07, 1.849e-05),
            ]
        )
    )
    ...
    ice_cube_hese[:, 2] = 10 ** ice_cube_hese[:, 1] - 10 ** (
        ice_cube_hese[:, 1] - ice_cube_hese[:, 2]
    )
    ice_cube_hese[:, 3] = (
        10 ** (ice_cube_hese[:, 1] + ice_cube_hese[:, 3]) - 10 ** ice_cube_hese[:, 1]
    )

    ice_cube_hese[:, 0] = 10 ** ice_cube_hese[:, 0] * units.GeV
    ice_cube_hese[:, 1] = 10 ** ice_cube_hese[:, 1] * (
        units.GeV * units.cm**-2 * units.second**-1 * units.sr**-1
    )
    ice_cube_hese[:, 1] *= 3
    ...
    def get_ice_cube_mu_range():
        ...
        upper = np.maximum(
            ice_cube_nu_fit(energy, offset=1.01, slope=-2.09),
            ice_cube_nu_fit(energy, offset=1.27, slope=-2.19),
            ice_cube_nu_fit(energy, offset=1.27, slope=-2.09),
        )  # ICRC
        upper *= energy**2
        #     lower = np.minimum(ice_cube_nu_fit(energy, offset=0.9, slope=-2.26),
        #                        ice_cube_nu_fit(energy, offset=0.63, slope=-2.13)) #ApJ
        lower = np.minimum(
            ice_cube_nu_fit(energy, offset=1.01, slope=-2.29),
            ice_cube_nu_fit(energy, offset=0.78, slope=-2.19),
        )  # ICRC
        lower *= energy**2
    ...
    def get_ice_cube_hese_range():
        energy = np.arange(1e5, 5e6, 1e5) * units.GeV
        upper = np.maximum(
            ice_cube_nu_fit(energy, offset=2.46, slope=-2.63),
            ice_cube_nu_fit(energy, offset=2.76, slope=-2.92),
        )
        upper *= energy**2
        lower = np.minimum(
            ice_cube_nu_fit(energy, offset=2.46, slope=-3.25),
    ...
    # ARA 4 years https://journals.aps.org/prd/abstract/10.1103/PhysRevD.102.043021
    ara_4year = np.array(
        (
            [1.00e16, 175630.000],
            [3.16e16, 10212.86886],
    ...
    ara_4year[:, 1] *= units.eV * units.cm**-2 * units.second**-1 * units.sr**-1
    ara_4year[:, 1] /= 1  # full-decade binning
    ...
    arianna[:, 1]  # full decade binning according to Brian
    arianna[:, 1] *= energyBinsPerDecade

    # Theoretical models

    # Diffuse Flux
    Heinze_band = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__), "data_gen2_limit_figure/talys_neu_bands.out"
        )
    )
    ...
    Heinze_band_int = interp1d(
        Heinze_band[:, 0], Heinze_band[:, 1] * Heinze_band[:, 0] ** 2
    )

    Heinze_evo = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__), "data_gen2_limit_figure/talys_neu_evolutions.out"
        )
    )
    ...
    Heinze_evo_int = interp1d(
        Heinze_evo[:, 0], Heinze_evo[:, 6] * Heinze_evo[:, 0] ** 2
    )

    vanVliet_max_1 = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__), "data_gen2_limit_figure/MaxNeutrinos1.txt"
        )
    )
    vanVliet_max_2 = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__), "data_gen2_limit_figure/MaxNeutrinos2.txt"
        )
    )
    ...
    vanVliet_reas = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__), "data_gen2_limit_figure/ReasonableNeutrinos1.txt"
        )
    )
    ...
    tde = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__), "data_gen2_limit_figure/TDEneutrinos.txt"
        )
    )
    tde[:, 0] *= units.GeV
    ...
    ll_grb = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__), "data_gen2_limit_figure/LLGRBneutrinos.txt"
        )
    )
    ll_grb[:, 0] *= units.GeV
    ...
    pulsars = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__), "data_gen2_limit_figure/Pulsar_Fang+_2014.txt"
        )
    )
    pulsars[:, 0] *= units.GeV
    ...
    pulsars_int = interp1d(
        pulsars[:, 0], np.average((pulsars[:, 1], pulsars[:, 2]), axis=0)
    )

    clusters = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__),
            "data_gen2_limit_figure/cluster_Fang_Murase_2018.txt",
        )
    )
    ...
    grb_afterglow = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__),
            "data_gen2_limit_figure/grb_ag_wind_optimistic.dat",
        )
    )
    ...
    agn = np.loadtxt(
        os.path.join(
            os.path.dirname(__file__), "data_gen2_limit_figure/agn_murase.dat"
        ),
        delimiter=",",
    )
    agn[:, 0] = agn[:, 0] * units.GeV
    agn[:, 1] = (
        agn[:, 1] * units.GeV * units.cm**-2 * units.second**-1 * units.sr**-1
    )
    ...
    def get_E2_limit_figure(
        ...
        max_all_models = np.maximum(
            vanVliet_max_int(plot_energy), Heinze_evo_int(plot_energy)
        )
        #
        max_plot = ax.fill_between(
            plot_energy / plotUnitsEnergy,
            max_all_models / plotUnitsFlux,
            1e-11,
            color="0.9",
            label="From UHECR, allowed region",
        )

        (best_fit_plot,) = ax.plot(
            plot_energy / plotUnitsEnergy,
            Heinze_band_int(plot_energy) / plotUnitsFlux,
            c="0.4",
            linestyle="--",
            label="From UHECR, best fit",
        )

        (agn_plot,) = ax.plot(
            plot_energy / plotUnitsEnergy,
            agn_int(plot_energy) / plotUnitsFlux,
            c="0.4",
            linestyle="-.",
            label=agn_label,
        )
        ...
        first_legend = plt.legend(
            handles=[agn_plot, best_fit_plot, max_plot], loc=4, fontsize=legendfontsize
        )
        ...
        if show_grand_10k:
            # 5 years scaling
            ax.plot(
                GRAND_energy / plotUnitsEnergy,
                GRAND_10k * 3 / 5.0 / plotUnitsFlux,
                linestyle=":",
                color="saddlebrown",
            )
        ...
        if show_grand_200k:
            ...
            # 10 years
            ax.plot(
                GRAND_200k_10[:, 0] / plotUnitsEnergy,
                GRAND_200k_10[:, 4] / plotUnitsFlux,
                linestyle=":",
                color="saddlebrown",
            )
            ax.annotate(
        ...
        if show_radar:
            ax.fill_between(
                Radar[:, 0] / plotUnitsEnergy,
                Radar[:, 1] / plotUnitsFlux,
                Radar[:, 2] / plotUnitsFlux,
                facecolor="None",
                hatch="x",
                edgecolor="0.8",
            )
        ...
        if show_ice_cube_EHE_limit:
            ax.plot(
                ice_cube_limit[2:, 0] / plotUnitsEnergy,
                ice_cube_limit[2:, 1] / plotUnitsFlux,
                color="dodgerblue",
            )
            if energyBinsPerDecade == 2:
                ax.annotate(
        ...
        if show_ice_cube_HESE_data:
            ...
            uplimit[np.where(ice_cube_hese[:, 3] != 0.0)] = 0

            ax.errorbar(
                ice_cube_hese[:, 0] / plotUnitsEnergy,
        ...
        if show_ice_cube_HESE_fit:
            ...
            ax.fill_between(
                ice_cube_hese_range[0] / plotUnitsEnergy,
                ice_cube_hese_range[1] / plotUnitsFlux,
                ice_cube_hese_range[2] / plotUnitsFlux,
                hatch="//",
            ...
            plt.plot(
                ice_cube_hese_range[0] / plotUnitsEnergy,
                ice_cube_nu_fit(ice_cube_hese_range[0], offset=2.46, slope=-2.92)
                * ice_cube_hese_range[0] ** 2
                / plotUnitsFlux,
                color="dodgerblue",
            )
        ...
        if show_ice_cube_mu:
            ...
            ax.fill_between(
                ice_cube_mu_range[0] / plotUnitsEnergy,
                ice_cube_mu_range[1] / plotUnitsFlux,
                ice_cube_mu_range[2] / plotUnitsFlux,
                hatch="\\",
                edgecolor="dodgerblue",
                facecolor="azure",
                zorder=2,
            ...
            plt.plot(
                ice_cube_mu_range[0] / plotUnitsEnergy,
                ice_cube_nu_fit(ice_cube_mu_range[0], offset=1.01, slope=-2.19)
                * ice_cube_mu_range[0] ** 2
                / plotUnitsFlux,
                color="dodgerblue",
            )
            ...
            plt.plot(
                energy_placeholder / plotUnitsEnergy,
                ice_cube_nu_fit(energy_placeholder, offset=1.01, slope=-2.19)
                * energy_placeholder**2
                / plotUnitsFlux,
                color="dodgerblue",
                linestyle="--",
            )
        ...
        if show_anita_I_III_limit:
            ax.plot(
                anita_limit[:, 0] / plotUnitsEnergy,
                anita_limit[:, 1] / plotUnitsFlux,
                color="darkorange",
            )
        ...
        if show_auger_limit:
            ax.plot(
                auger_limit[:, 0] / plotUnitsEnergy,
                auger_limit[:, 1] / plotUnitsFlux,
                color="forestgreen",
            )
            if energyBinsPerDecade == 2:
                ax.annotate(
        ...
        if show_ara:
            ...
            ax.plot(
                ara_4year[:, 0] / plotUnitsEnergy,
                ara_4year[:, 1] / plotUnitsFlux,
                color="indigo",
                linestyle="-",
            )
            if energyBinsPerDecade == 2:
        ...
        if show_arianna:
            ax.plot(
                arianna[:, 0] / plotUnitsEnergy,
                arianna[:, 1] / plotUnitsFlux,
                color="deeppink",
            )
            ax.annotate(
                "ARIANNA",
    ...
    pyrex_60 = np.array(
        (
            ...
            [17.0, 1.290e0, 5.528e-2],
            [17.5, 3.250e0, 1.396e-1],
            [18.0, 7.013e0, 2.509e-1],
            [18.5, 1.249e1, 4.185e-1],
            [19.0, 2.290e1, 5.823e-1],
            [19.5, 4.273e1, 9.290e-1],
    ...
    pyrex_100_old = np.array(
        (
            ...
            [17.0, 1.717e0, 6.563e-2],
            [17.5, 4.931e0, 1.719e-1],
            [18.0, 1.056e1, 3.163e-1],
            [18.5, 1.971e1, 5.258e-1],
            [19.0, 3.530e1, 7.036e-1],
            [19.5, 6.863e1, 1.177e0],
    ...
    pyrex_100 = np.array(
        (
            ...
            [17.0, 1.42276e00, 5.65013e-02],
            [17.5, 3.71839e00, 1.37013e-01],
            [18.0, 7.55479e00, 2.60396e-01],
            [18.5, 1.47662e01, 4.55059e-01],
            [19.0, 3.07369e01, 6.56543e-01],
            [19.5, 5.79992e01, 1.08224e00],
            [20.0, 9.06216e01, 1.35279e00],
        )
    )
...
#THIS IS THE GENERIC PATHFINDER AT SOUTHPOLE, shaded region for uncertainties in trigger, design etc.
    ...
    if show_prediction_arianna_200:
        ...
        arianna_200 = np.loadtxt("data_gen2_limit_figure/expected_sensivity_ARIANNA-200.txt")
