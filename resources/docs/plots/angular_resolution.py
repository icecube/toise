...
with plotting.pretty():
    ...
    def plot_psf(ax, emu, cos_theta=0):
        ...
        ax.add_artist(
            AnchoredText(
                r"$\cos\theta=%.1f$, $E_{\mu}=$%s"
                % (cos_theta, plotting.format_energy("%d", emu)),
                loc=2,
                frameon=False,
            )
        )
    ...
    def plot_median_opening_angle(ax, cos_theta=0, legend=False):
        ...
        ax.add_artist(
            AnchoredText(r"$\cos\theta=%.1f$" % (cos_theta), loc=3, frameon=False)
        )
        ax.set_ylim(0, 0.8)
        ax.grid(True)
    ...
    plot_psf(plt.subplot(griddy[0]), 1e5, 0.0)
    plot_psf(plt.subplot(griddy[1]), 1e5, -0.3)
    plot_median_opening_angle(plt.subplot(griddy[2]), 0.0)
    plot_median_opening_angle(plt.subplot(griddy[3]), -0.3)

    fig.axes[0].legend(
        frameon=True, framealpha=0.8, loc="lower right"
