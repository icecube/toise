...
def create_aeff(geometry='IceCube', spacing=200,
    veto_threshold=1e4, veto_area=0., energy_threshold=None, no_cuts=False,
    psf_spacing=None, angular_resolution_scale=1., **kwargs):
    
    if psf_spacing is None:
        psf_spacing = spacing
    
    if veto_area > 0:
        kwargs['veto_coverage'] = surface_veto.GeometricVetoCoverage(geometry, spacing, veto_area)
    
    if energy_threshold is not None:
        seleff = effective_areas.get_muon_selection_efficiency(geometry, spacing, energy_threshold=energy_threshold)
    else:
    ...
    return effective_areas.create_throughgoing_aeff(
        ...
        psf=angular_resolution.get_angular_resolution(geometry, psf_spacing, scale=angular_resolution_scale),
...
def demo_plot(geo='Sunflower', spacing=240, sindec=0.1, livetime=1., angular_resolution_scale=1.):
    ...
    with plotting.pretty():
        ...
        ax.plot(*plotting.stepped_path(psi, dp*ps.expectations()['tracks'].sum(axis=0)), label=r'$%.2f \times 10^{-12} \rm \,\, TeV \, cm^{-2} \, s^{-1}$' % dp)
        ...
        bkg = background['atmo'].expectations['tracks'].sum(axis=0) + \
            background['astro'].expectations(gamma=fixed['gamma'])['tracks'].sum(axis=0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sin-dec", type=float, default=0.)
    parser.add_argument("--angular-resolution-scale", type=float, default=1.)
    opts = parser.parse_args()
    
    import pylab
    demo_plot(geo=opts.geometry, spacing=opts.spacing, sindec=opts.sin_dec, livetime=opts.livetime, angular_resolution_scale=opts.angular_resolution_scale)
    pylab.show()