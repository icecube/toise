...
def create_psf(fname='FinalLevel_IC86.2011_muongun.003333.hdf5', muon='MCMuon', reco='SplineMPEMuEXDifferential', cut=None):
	import dashi
	from cubicle.hdfweights import HDFScanner
	hdf = HDFScanner(fname, primary='/'+muon, type='unweighted')
	q = hdf(primary='log10({muon}.energy)'.format(**locals()),
	        ct='cos({muon}.zenith)'.format(**locals()),
	        alpha='opening_angle({muon}, {reco})/I3Units.degree'.format(**locals()),
	        where=cut)
	h = q.hist(fields=('primary', 'ct', 'alpha',), bins=(numpy.linspace(3, 8, 51), numpy.linspace(-1, 1, 21), numpy.linspace(0, 25, int(25e2))))
	del q
	del hdf
...
def fit_psf(h, smooth=1e-6):
	...
	power = [1., 1., 2.]
	nknots = [25, 25, 25]
	knots = [pad_knots(numpy.linspace(e[0]**(1./p), e[-1]**(1./p), n)**p, o) for n, o, p, e in zip(nknots, order, power, h.binedges)]
	
	z = h.bincontent.cumsum(axis=2)
	w = 1./h.squaredweights.cumsum(axis=2)
	w[~numpy.isfinite(w)] = 0.
	
	del h
	
	penalties = defaultdict(lambda : [0.]*len(order))
...
def plot_psf(h, spline, cos_theta=0):
	...
	for logE in numpy.arange(4, 8):
		...
		x = sub.bincenters
		y = glam.grideval(spline, [[h.bincenters[0][ei-1]], [h.bincenters[1][zi-1]], x]).flatten()
...
if __name__ == "__main__":
	...
	parser.add_argument('--cos-theta', type=float, default=0.,
	    help='zenith band to plot')
	...
	if opts.plot:
		...
		spline = fit_psf(create_psf(opts.hdf_file, opts.muon, opts.reco, opts.cut), opts.smooth)
