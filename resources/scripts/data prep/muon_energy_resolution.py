...
def plot_energy_resolution(h2, axis_range=(2.5, 7)):
    ...
    with plotting.pretty(tex=False):
        ...
        left_grid = gridspec.GridSpecFromSubplotSpec(2,1, subplot_spec=outer_grid[0], height_ratios=[1,10])
        ...
        ax.errorbar(10**sp.x, 10**sp.y,
                    yerr=(10**sp.y - 10**(sp.y - sp.yerr), 10**(sp.yerr + sp.y) - 10**sp.y), color='k',
                    capsize=2, ls=':')
...
def save_energy_resolution_profile(h2, fname, smoothing=1e-2):
    ...
    std = numpy.concatenate((sp.yerr, numpy.ones(sp.yerr.size-1)*(sp.yerr[-10:].mean())))
