...
constant_offset = pd.Series(
    nonnull.iloc[0] + (nonnull.index[0] - dats.index), index=dats.index
)
...
np.savez(
    "energy_resolution_{}.npz".format(the_geom),
    loge=dats.index,
    mean=dats.index + dats["mean"],
    std=dats["std"],
    smoothing=1e-2,
)
