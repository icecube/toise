...
class Winter2014GRBFluence(object):
    ...
    def __init__(self):
        ...
        energy, diffuse_flux = np.loadtxt(StringIO(
            """74.865	3.105e-16
		    185.820 1.270e-15
		    669.845 7.765e-15
		    4902.930	9.101e-14
    ...
    def __call__(self, E, Eiso=1e53, z=2):
        ...
        geometric_scale = (self.dl(2.)/self.dl(z))**2
...
class GRBPopulation(pointsource.PointSource):
    def __init__(self, effective_area, z, Eiso, with_energy=True):
        ...
        pointsource.PointSource.__init__(self, effective_area, fluence.sum(
            axis=1)*0.9/nbands, slice(None), with_energy)
...
class LuminosityDistance(object):
    ...
    def __init__(self, zmax=10, steps=1000, O_M=0.3, O_L=0.7, w=-1,
                 v_dip=[0, 0, 0], v_mon=0, coords=[0, 0],
                 Flat=True, EOS=False, H_0=70,
                 der=False, dip=False):
        """
        ...
        # H_rec = 1/(H(z)/H_0)
        if EOS:
            def H_rec(x): return 1/np.sqrt(O_M*(1+x)**3+(O_K)*(1+x)**2
                                           + O_L*(1+x)**(3*(1+w)))
        else:
            def H_rec(x): return 1/np.sqrt(O_M*(1+x)**3+(O_K)*(1+x)**2
                                           + O_L)
        ...
        integral = integrate.cumtrapz(H_rec(z), z, initial=0.)

        if O_K == 0:
            result = c*(1+z)/H_0*integral
        elif O_K < 0:
            result = c*(1+z)/H_0/np.sqrt(-O_K)*np.sin(np.sqrt(-O_K)*integral)
        else:
        ...
        if dip:
            if not Flat:
                warnings.warn(
                    'dipole not properly implemented for non-flat cosmology')
            cos_theta = np.sin(coords[0])*np.sin(v_dip[1]) *\
                np.cos(coords[1]-v_dip[2]) +\
...
def grb_rate(z):
    ...
    """
    return (grb_density(z)*units.cm**3) * comoving_volume(z) / (1. + z)
...
def isotropic_equivalent_energy(l_peak, t90=10.):
    """
...
def comoving_volume(z):
    ...
    return DH * (1.+z)**2 * angular_distance(z)**2 / scale(z)
