...
class AngularResolution(object):
    ...
    def median_opening_angle(self, energy, cos_theta):
        ...
        mu_reco = self._spline.ev(
            loge.flatten(), ct.flatten()).reshape(loge.shape)
...
class PointSpreadFunction(object):
    ...
    def __call__(self, psi, energy, cos_theta):
        psi, loge, ct = numpy.broadcast_arrays(numpy.degrees(
            psi)/self._scale, numpy.log10(energy), cos_theta)
...
class SplineKingPointSpreadFunction(KingPointSpreadFunctionBase):
    def __init__(self, fname='Sunflower_240_kingpsf1', **kwargs):
        ...
        self._splines = dict(sigma=SplineTable(
            fname + '.sigma.fits'), gamma=SplineTable(fname + '.gamma.fits'))
    ...
    def get_params(self, log_energy, cos_theta):
        ...
        sigma = 10**self._splines['sigma'].evaluate_simple(
            [log_energy, cos_theta])
        gamma = 10**self._splines['gamma'].evaluate_simple(
            [log_energy, cos_theta]) + 1
