...
def make_cylinder(volume=1., aspect=1.):
...
def base_aeff():
    ...
    (e_nu, cos_theta, e_mu), efficiency = effective_areas.get_muon_production_efficiency(cos_theta)
...
def get_aeff(angular_resolution=0.5, energy_threshold=1e3):
    ...
    psi_bins = numpy.concatenate((sqrt(linspace(0, numpy.radians(angular_resolution*4)**2, 101)), [numpy.inf]))
    
    # Step 4: apply smearing for angular resolution
    cdf = psf(psi_bins[:-1], 0, 0)
    smear = numpy.concatenate((diff(cdf), [1.-cdf[-1]]))
    
    expand = [None]*4 + [slice(None)]
    
    edges = (e_nu, cos_theta, e_mu[idx:], psi_bins)
    
    return effective_areas.effective_area(edges, base.values[...,idx:,None] * smear[expand], 'cos_theta')
...
def discovery_potential(angular_resolution=0.5, energy_threshold=1e3, area=1):
    ...
    livetime = 10.*area
    aeff = get_aeff(angular_resolution=angular_resolution, energy_threshold=energy_threshold)
    atmo_bkg = diffuse.AtmosphericNu.conventional(aeff, livetime).point_source_background(0)
