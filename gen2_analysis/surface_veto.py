...
def get_geometric_coverage_for_area(geometry, spacing, area, ct_bins=numpy.linspace(0, 1, 11), nsamples=int(1e4)):
    ...
    for i, (ct_lo, ct_hi) in enumerate(zip(ct_bins[:-1], ct_bins[1:])):
        ...
        coverage[i] = sum(
            map(veto_surface.point_in_footprint, pos))/float(nsamples)
    return coverage


class GeometricVetoCoverage(object):
    cache_file = os.path.join(
        data_dir, 'veto', 'geometric_veto_coverage.pickle')
    ...
    def __call__(self, ct_bins=numpy.linspace(-1, 1, 11)):
        key = (self.geometry, self.spacing, self.area,
...
class EulerVetoProbability(object):
    ...
    def __call__(self, energy, cos_theta):
        logE, cos_theta = numpy.broadcast_arrays(numpy.log10(
            energy/1e3), numpy.clip(cos_theta, -1, self.ct_max))
...
def bundle_energy_distribution(emu_edges, eprim, a=1, cos_theta=1., depth=1950.):
    ...
    :param depth: vertical depth of detector
    """
    surface_energy, mean_loss = bundle_energy_at_depth(
        eprim, a, cos_theta, depth)
    ...
    sigma = numpy.minimum(1.25 - 0.16*numpy.log10(eprim) +
                          0.00563*numpy.log10(eprim)**2, 1.)
...
class MuonBundleBackground(object):
    def __init__(self, effective_area, livetime=1.):
        ...
        flux = numpy.stack([bundle_flux_at_depth(emu, ct)[0][:, :4, :].sum(
            axis=(1, 2)) for ct in center(cos_theta)]).T
