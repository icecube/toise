...
class TernaryAxes(Axes):
    ...
    def get_xaxis_text1_transform(self, pad_points):
        ...
        return (self.transData +
                ScaledTranslation(pad*(np.sin(angle) + 0.5*np.sin(angle-np.pi/3)), pad*(-np.cos(angle) - 0.5*np.cos(angle-np.pi/3)),
...
def flavor_triangle(fig=None, subplotspec='111', grid=False):
    ...
    for subax in (ax.ab, ax.bc, ax.ca):
        ...
        plt.setp(subax.xaxis.get_majorticklabels(),
                 rotation=rot, va='center', ha='center')
    ...
    ax.bc.set_xlabel(
        r'$f_{\mu}$', fontdict=fontdict).set_position((0.5, -0.07))
    ax.ca.set_xlabel(
        r'$f_{\tau}$', fontdict=fontdict).set_position((0.5, -0.1))
    ...
    ax.ab.scatter([0.93/3], [1.05/3], marker='o', facecolor='k',
                  edgecolor='w', lw=0.5, s=20, label='1:2:0').set_zorder(100)
    ax.ab.scatter([0.19], [0.43], marker='s', facecolor='k',
                  edgecolor='w', lw=0.5, s=20, label='0:1:0').set_zorder(100)
    ax.ab.scatter([0.55], [0.19], marker='^', facecolor='k',
                  edgecolor='w', lw=0.5, s=20, label='1:0:0').set_zorder(100)
