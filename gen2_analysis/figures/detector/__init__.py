...
def surface_geometry():
    ...
    def get_string_heads(geometry, spacing, **kwargs):
        with gzip.GzipFile(surfaces.get_geometry_file(geometry, spacing)) as f:
            geo = np.loadtxt(f, dtype=np.dtype([('string',int), ('om',int)] + [(c,float) for c in 'xyz']))
            pos = geo[geo['om'] == 1]
            return pos[list('xy')]
    ...
    axes[0].scatter(radio[:,0], radio[:,1], s=5, color='C4', label='Gen2-Radio', marker='1')
    ...
    axes[1].scatter(pos[86:]['x'], pos[86:]['y'], s=5, color='C1', label='IceCube-Gen2', marker='o')

    axes[2].scatter(pos[:86]['x'], pos[:86]['y'], s=5, color='C0', label='IceCube', marker='H')

    axes[3].scatter(upgrade[:,0], upgrade[:,1], s=10, color='C3', label='IceCube Upgrade', marker='P')
    ...
    for ax in axes:
        ...
        ax.legend(loc='lower center', bbox_to_anchor=(0.5,1), frameon=False, handletextpad=0.1, markerscale=3)
    ...
    for parent, child in reversed(zip(axes[:-1], axes[1:])):
        inset_locator.mark_inset(parent, child, 2, 3, edgecolor='black', alpha=0.3, ls='-')
