...
@figure_data()
def single_flare_time_to_signficance(exposures, flux=1.6e-15, gamma=-2.1, dec=5.69, emin=0., emax=np.inf):
    """
    ...
    flux_norm = flux*(100.**-gamma)/1e-12
    ...
    def ts(flare_time, total_time):
        ...
        return {'ts': -2*(allh.llh(**null)-allh.llh(**alternate)-np.log(flare_time/total_time)), 'ns': ns, 'nb': nb}
        # return -2*(allh.llh(ps=0, **fixed)-allh.llh(ps=flux_norm, **fixed))
    
    days = [3, 10, 20, 50, 100, 158, 200, 300, 500]
    return {
        'days': days,
        'ts': [ts(dt/365.0, exposures[0][1]) for dt in days]
    }
...
def single_flare_time_to_signficance(datasets):
    ...
    for dataset in datasets:
        ...
        mask = corrected_ts > 0
        mask[-1] = False
        line = plt.plot(np.asarray(x)[mask], np.sqrt(corrected_ts)[mask], label=label)[0]
